...
function configurarCatalogos() {
    ...
    criarOpcoes(tiposServico, 'serviceChoices', 'serviceChoice');
...
function mostrarAbaReparo(aba) {
    document.getElementById('partsPanel').style.display = aba === 'pecas' ? '' : 'none';
    document.getElementById('servicesPanel').style.display = aba === 'servicos' ? '' : 'none';
    document.getElementById('tabParts').classList.toggle('active', aba === 'pecas');
    document.getElementById('tabServices').classList.toggle('active', aba === 'servicos');
}
...
function preencherNomePeca() { document.getElementById('usedPart').value = [document.getElementById('partType').value, document.getElementById('partVariant').value].filter(Boolean).join(' '); }
...
function editarOS(osId) {
    ...
    marcarSelecionados('serviceChoice', os.servicosSelecionados || []);
    document.getElementById('otherService').value = os.outroServico || '';
...
async function salvarOS(event) {
    ...
    const novaOS = {
        ...
        servicosSelecionados: selecionados('serviceChoice'),
        outroServico: document.getElementById('otherService').value.trim(),
...
function atualizarPainel() {
    ...
    ordensFiltradas.forEach((os) => {
        ...
        card.innerHTML = `
            ...
            <p>🛠️ <strong>Serviços:</strong> ${[...(os.servicosSelecionados || []), os.outroServico].filter(Boolean).join(', ') || 'Nenhum registrado'}</p>
...
async function waEnviarComprovante(osId) {
    ...
    texto += `🛠️ *Serviços:* ${[...(os.servicosSelecionados || []), os.outroServico].filter(Boolean).join(', ') || 'Nenhum registrado'}\n`;
...
async function exportarBackup() {
    ...
    try {
        ...
        const [ordens, estoque] = await Promise.all([db.collection('ordens_servico').get({source:'server'}), db.collection('estoque_pecas').get({source:'server'})]);
        const arquivo = {
        ...
        mensagem.textContent = `Backup gerado: ${arquivo.ordens.length} OS e ${arquivo.estoque.length} peças. Guarde o arquivo em local privado.`;
...
async function restaurarBackup() {
    ...
    try {
        ...
        if (dados.ordens.some(o => !o || typeof o.idOS !== 'string') || dados.estoque.some(p => !p || typeof p.nome !== 'string')) throw new Error('Registros inválidos no backup.');
        const digitado = prompt(`Backup de ${dados.ordens.length} OS e ${dados.estoque.length} peças. Apenas registros ausentes serão adicionados; existentes não serão substituídos. Digite RESTAURAR para continuar:`);
        if (digitado !== 'RESTAURAR') { mensagem.textContent = 'Restauração cancelada.'; return; }
        const [snapOS, snapEstoque] = await Promise.all([db.collection('ordens_servico').get({source:'server'}), db.collection('estoque_pecas').get({source:'server'})]);
        ...
        for (const [colecao, itens, ids, chaves, chave] of [
            ...
            for (const item of itens) {
                ...
                if ((item.idDoc && ids.has(item.idDoc)) || chaves.has(key)) {
